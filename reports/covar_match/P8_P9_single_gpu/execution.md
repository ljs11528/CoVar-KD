...
状态：截至 2026-09-12 10:24（北京时间），三条单卡 CE-only 80k 训练全部完成并通过独立核验；第二模型对的 15 条第一阶段 KD 正在执行，尚未形成完整温度比较。正式队列于 2026-09-11 21:05 启动，控制进程 PID 2776。


## 本次 CE-only 完整结果

三 seed 的 80k final mIoU 均值为 **61.845306%**，样本标准差为 **0.188929 个百分点**。均值只包含本次单卡 fresh 80k 结果。

| seed | 20k mIoU (%) | 40k | 60k | 80k |
|---:|---:|---:|---:|---:|
...
![CE-only validation trajectories](P8_ce_only_trajectory.png)

核验覆盖三条 run 的 80k 日志、全部协议字段、12 个验证点、12 份里程碑状态和训练结束记录。JSON、CSV 与原日志一致，均值及样本标准差已独立重算，启动时记录的训练源文件和权重 SHA-256 均保持一致。证据见 [P8_completion_audit.json](P8_completion_audit.json)，完整报告见 [P8_ce_only.md](P8_ce_only.md)，逐点数据见 [P8_ce_only.csv](P8_ce_only.csv)。

旧 P7 七个温度的均值在数值上均高于本次 CE 均值，跨协议均值差为 +0.377129 至 +0.903122 个百分点。该描述同时包含 KD 和单卡/双卡协议变化，不能据此认定七个温度都改善了监督训练或识别负迁移。第二模型对第一阶段当前运行 T=0.25、seed 1234；补点决策将在 15 条基础 KD run 完成后执行。
...
本机下载目录：/Users/ljs/research_2026/covar_kd_2026/remote/downloads/P8_P9_single_gpu/。P8 结果已完成核验；P9 结论将在既定温度网格完成后更新。
