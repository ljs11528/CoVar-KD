...
- 制定日期：2026-07-13
- 当前状态：预注册已冻结，尚未实现 O1.2，尚未启动学生训练
...
当前不授权 O1.2 学生训练。必须先完成独立实现、单元测试、全量 train/val 机制诊断和联合门禁。
...
下一项唯一允许的工程动作是：在不修改 O1.1 冻结源码的前提下，实现独立 O1.2 module、diagnose、checker 和 tests，然后停在 O1.2-A 联合门禁供人工审查。
