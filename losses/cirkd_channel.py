...
class StudentSegChannelContrast(nn.Module):
    ...
    def _sample_negative(self, Q, index):
        ...
        device = Q.device
        X_ = torch.zeros((class_num * contrast_size, feat_size), device=device).float()
        y_ = torch.zeros((class_num * contrast_size, 1), device=device).float()
        sample_ptr = 0
    ...
    def forward(self, feat_S, feat_T):
        ...
        sim_dis = torch.tensor(0., device=feat_S.device)
        for i in range(B):
            for j in range(B):
