...
echo "[covar-npu] Phase C triage started at $(date -Is)"
...
REPORT_PATH=${REPORT_PATH:-"$REPORT_DIR/$(date +%F)_phaseC_npu_triage.md"}
mkdir -p "$REPORT_DIR"
...
if [[ "${AUTO_GIT_REPORT:-1}" == "1" ]]; then
  ...
  if git diff --cached --quiet -- "$REPORT_PATH"; then
    ...
    git commit -m "Add Phase C NPU triage report"
...
echo "[covar-npu] Phase C triage finished at $(date -Is)"
