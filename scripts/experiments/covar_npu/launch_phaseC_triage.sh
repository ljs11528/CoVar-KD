...
RUN_LOG="$RUN_LOG_DIR/phaseC_triage_${MAX_ITERATIONS}.log"
...
if [[ -f "$PID_FILE" ]]; then
  ...
  if [[ -n "$old_pid" ]] && kill -0 "$old_pid" 2>/dev/null; then
    echo "Phase C triage already appears to be running: pid=$old_pid"
    echo "log=$RUN_LOG"
    exit 1
  fi
fi
...
setsid nohup bash scripts/experiments/covar_npu/run_phaseC_triage.sh > "$RUN_LOG" 2>&1 < /dev/null &
pid=$!
echo "$pid" > "$PID_FILE"

echo "Started Phase C NPU triage"
echo "pid=$pid"
