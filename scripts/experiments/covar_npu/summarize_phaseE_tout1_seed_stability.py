...
def parse_args():
    parser = argparse.ArgumentParser(description="Summarize Phase E Tout=1.0 seed stability.")
...
def parse_log(path):
    ...
    result["complete"] = result["last_iter"] == result.get("max_iterations")
    return result
...
def main():
    ...
    phase_d_root = Path(args.phase_d_root)
    seeds = args.seeds.split()

    rows = []
    for seed in seeds:
        for covar, variant in [
            ("off", "phaseD_cirkd_no_covar_tout1" if seed == "1234" else f"phaseE_seed{seed}_cirkd_no_covar_tout1"),
            ("on", "phaseD_covar_newton_gamma2_tout1" if seed == "1234" else f"phaseE_seed{seed}_covar_newton_gamma2_tout1"),
        ]:
            root = phase_d_root if seed == "1234" else phase_e_root
    ...
    lines = [
        ...
        f"- Existing seed-1234 root: `{phase_d_root}`",
        f"- Seeds: `{', '.join(seeds)}`",
        "",
        "## Per-Seed Results",
        "",
