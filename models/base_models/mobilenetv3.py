...
class MobileNetV2(nn.Module):
    ...
    def loadpretrainedweights(self, pretrained=''):
        if isinstance(pretrained, bool):
            use_default = pretrained
        else:
            use_default = False

        state_dict = None
        if isinstance(pretrained, str) and pretrained != 'None':
            if os.path.isfile(pretrained):
                checkpoint = torch.load(pretrained, map_location='cpu')
                state_dict = checkpoint['state_dict'] if 'state_dict' in checkpoint else checkpoint
            elif pretrained.startswith('http'):
                state_dict = load_state_dict_from_url(pretrained, map_location='cpu')
            elif pretrained in DEFAULT_MODEL_URLS:
                state_dict = load_state_dict_from_url(DEFAULT_MODEL_URLS[pretrained], map_location='cpu')
            else:
                # fallback to default url for this backbone
                use_default = True
        if state_dict is None and use_default:
            state_dict = load_state_dict_from_url(DEFAULT_MODEL_URLS['mobilenetv2'], map_location='cpu')
        if state_dict is None:
            raise FileNotFoundError('Failed to load MobileNetV2 pretrained weights; please provide a valid path, URL, or set pretrained=True')
    ...
    def loadpretrainedweights(self, pretrained):
        ...
        if isinstance(pretrained, str) and pretrained != 'None':
            if os.path.isfile(pretrained):
                ...
                use_default = True
        if state_dict is None and use_default:
            key = 'mobilenetv3_small' if self.arch_type == 'small' else 'mobilenetv3_large'
            state_dict = load_state_dict_from_url(DEFAULT_MODEL_URLS[key], map_location='cpu')
        keys = list(state_dict.keys())
        for key in keys:
            if key.startswith('backbone.'):
                value = state_dict.pop(key)
